...
if [[ "$mode" == "dev" && "$tkind" == "remote-repo" ]]; then
  ...
  remote_inner="cd '$dwt' && export CLAUDE_MISS_ROLE=feature PRIMARY_REPO='$drepo' WORKTREES_DIR=\"\$HOME/missclaude-worktrees\" BASE_BRANCH='$dbase' MISS_REPO_ROOT='$drepo' MISS_REPO_ID='$MISS_REPO_ID' MISS_WORKTREE='$dwt' MISS_FEATURE_BRANCH='$MISS_FEATURE_BRANCH' MISS_INTEGRATION_BRANCH='$dbase' MISS_PREVIEW_PORT='$MISS_PREVIEW_PORT' MISSWORK_HOOK=\"\$HOME/.miss-claude/prevent-misswork.py\" MISS_ROLE_CONTEXT=\"\$HOME/.miss-claude/miss-role-context.py\" MISS_AGENTS_ATTACHED=1 CLAUDE_CODE_DISABLE_MOUSE=1 && S=\"\$HOME/.miss-claude/miss-rails.settings.json\" && A=\"\$(python3 \"\$HOME/.miss-claude/miss-agents.py\")\" && { $C --settings \"\$S\" --agents \"\$A\" --continue --dangerously-skip-permissions || $C --settings \"\$S\" --agents \"\$A\" --dangerously-skip-permissions; }"
